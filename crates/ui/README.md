...
- `slint/app.slint`:整个界面的声明。三个 tab(卡片主页、音乐页、3D 页)、宽/紧凑
  两种版式(由宽度决定,`docs/adr/0007`)、常驻于音乐页的控制条,以及从控制条展开的
  播放页覆层(CONTEXT.md「播放页」):warp 视觉铺底、封面卡与歌名居中、底部复用
  ControlCluster 保证功能等价路径(`docs/adr/0010`)。
...
- `src/music.rs`:音乐页绑定。搜索/每日/红心、队列点播、控制条、自动续播、
  开机自检,以及播放页元数据(歌名/歌手/封面)的推送。显示格式化都在这层做。
- `src/viz.rs`:播放页可视化的 seam 数据(`VizControls`,POD)与音频载荷的取帧
  helper。wasm 侧数据源恒空,取帧代码无平台判断。
