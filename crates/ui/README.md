...
- `slint/app.slint`:整个界面的声明。两个 tab(卡片主页、音乐页)、宽/紧凑
  ...
  标注卡 → 遮挡层(裁到标注卡)→ 歌名与歌词 → 复用 `PlayerBar` 的功能层
...
- `slint/playerbar.slint`:播放控制条,**全应用唯一的一条**。四处实例:音乐页页尾、
  播放页覆层功能层、以及非音乐页那颗迷你胶囊(点非控件区就地展开)。功能分三面轮换
  —— 面 A 播放核心、面 B 模式与声音、面 C 同播与特效,翻面靠条右缘那三颗指示点。
  在此之前条身写了四遍,玻璃胶囊那套样式跟着抄了四遍(`docs/adr/0028`)。
  每面各有各的宽:三面共用一个钉死的宽度会让只有两三颗键的面摊在空胶囊里,
  而让条收内容宽收不出来 —— 三面的内容各自长在 `if` 里,条件子元素不给父元素
  贡献 preferred 尺寸。进度轨跨三面常驻:翻面可以藏起一簇键,不能藏起能力。
- `slint/controls.slint`:条上两块跨形态复用的积木 —— `RingControl`(环形进度播放键)
  与 `SyncStrip`(同播设备区,一台都没有时也常驻)。
- `slint/progress.slint`:横向进度条、音量、播放页那条竖向滑条。三者共用「按下即拖、
  松手才喊」的手势。细轨档的 `pinned` 是给触屏的:手机没有悬停,要指过来才现形的
  轨道在那里等于不存在。
