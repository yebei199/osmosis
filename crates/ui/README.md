...
- `slint/app.slint`:整个界面的声明。两个 tab(卡片主页、音乐页)、宽/紧凑
  ...
  歌名与歌词 → 复用 ControlCluster 的功能层(`docs/adr/0010`)。功能层那条
  控制条的底与两颗次要圆钮(视觉预设、收起)由 aurora 管线渲图贴回来
  (#69,底走 fluid、圆钮走 glass),尺寸由 `.slint` 回写。**深度卡片
...
- `src/nav_glass.rs`:导航选中器的 seam 数据与转场判定(静止时不重算纹理的
  工作量缓存,不是会冻住动画的门)。
...
- `src/wall_drive.rs`:卡墙的每帧驱动与 slint 绑定。指针/滚轮/双击变成
  相机与动画状态,封面缩略图取出像素、烘圆角后经 seam 上传;
  全部收敛即冻结,静止的墙零重绘。
