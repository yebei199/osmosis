...
- `slint/app.slint`:整个界面的声明。两个 tab(卡片主页、音乐页)、宽/紧凑
  ...
  播放页唯一的**深度卡片**是那块世界空间标注卡:锚点写在封面点云的局部坐标里,
  由 render3d 每帧投回视口(`viz-anchor-x/y/visible`),点云被拖着自转时卡片跟着
  走,并被更近的粒子逐像素挡住。歌词不算,它画在粒子之上(`docs/adr/0010` 的
  「歌词是例外」)。无 GPU 端粒子为空图、锚点恒无,自动退回 warp 形态。
