...
impl Execution {
    ...
    /// 本机换了一批、新的那一版还在发布的路上:旧批的条目号作废,队列标识留着
    /// (发布要拿它发新版本)。
    ///
    /// 不作废的话,主端这几百毫秒里写出的计划是「新歌 + 旧批同一位置的条目号」,
    /// 跟随端照条目号取到的是旧批里的另一首(#142)。作废之后主端写不出计划,
    /// 跟随端照手上那份接着放,发布回来 [`Self::adopt`] 一换,全组一起换歌。
    pub(in crate::music) fn forget_entries(&self) {
        self.inner.borrow_mut().entry_ids.clear();
    }
