        ...
        match api::liked_ids().await {
            ...
            Err(err) => {
                if let Some(ui) = weak.upgrade() {
                    crate::pages::account::handle_session_expiry(
                        &ui, &err,
                    );
                }
...
fn bump_liked_count(ui: &MainWindow, delta: i32) {
    ...
    let liked =
        crate::library::playlist::Source::Liked.to_index();
    ...
    for index in 0..rows.row_count() {
        ...
        let Some(count) =
            crate::library::playlist::track_count_of(
                &row.subtitle,
            )
        else {
            return;
        ...
        row.subtitle =
            crate::library::playlist::track_count_text(
                (count + delta).max(0),
            )
            .into();
