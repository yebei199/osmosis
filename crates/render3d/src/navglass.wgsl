...
fn base_color(uv: vec2<f32>) -> vec3<f32> {
    if (u.dark < 0.5) {
        // 浅色:亮暖底,而三团光斑改成**压暗**而不是提亮 —— 在亮底上叠饱和色
        // 只会冲成一片白,极光的结构就没了;减法保留同一处的明暗节奏,
        // 玻璃仍有东西可折射。底色与 slint 那侧的 Theme.aurora-base 同一个值。
        var l = vec3<f32>(0.953, 0.925, 0.882); // #f3ece1
        l -= vec3<f32>(0.13, 0.10, 0.06) * glow(uv, vec2<f32>(0.35, 0.22), 0.55);
        l -= vec3<f32>(0.10, 0.08, 0.05) * glow(uv, vec2<f32>(0.62, 0.55), 0.55);
        l -= vec3<f32>(0.06, 0.05, 0.03) * glow(uv, vec2<f32>(0.45, 0.85), 0.50);
        return l;
    }
