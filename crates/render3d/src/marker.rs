...
///
/// z 取封面平面(`CLOUD_ORIGIN.z`)之前 0.8:配上 [`ORBIT_RADIUS`],
/// 轨道的 z 就落在 1.9~3.3,近端离平面 1.5(高过粒子位移峰值 1.2~1.5,
/// 卡片干净),远端只离 0.1(贴着平面,粒子成片穿过)。
pub(crate) const ORBIT_CENTER: Vec3 =
    Vec3::new(0.0, 0.5, 2.6);

/// 轨道半径。
///
/// 0.7 远小于竖屏可见半宽 1.156(相机竖向 45°、平面在镜头前 6.2、长宽比 0.45),
/// 所以整条轨道都在画面里,卡片不会转着转着闪掉。
pub(crate) const ORBIT_RADIUS: f32 = 0.7;
...
pub(crate) fn front_face(pose: &Transform) -> Vec3 {
    pose.translation + Vec3::Z * MARKER_HALF
}
