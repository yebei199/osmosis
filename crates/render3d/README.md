...
- **锚点是世界空间的**,写在点云自己的局部坐标里(`CARD_ANCHOR_LOCAL`,封面平面上
  离中心 0.8 的那一点)。每帧由点云 root 的位姿变换成世界点、再由
  `Camera::world_to_ndc` 投一次,**一次投影两个去处**:z 当遮挡层的深度门槛,xy 归一成
  视口坐标交给 UI 当卡片挂点。点云被拖着自转时锚点跟着转,卡片于是跟着物体走 ——
  这就是「3D 物体的标注卡片」这个形态(`docs/note/vision.md`)。
  位姿用的是**这一帧刚写进 root 的旋转**,不读 `GlobalTransform`:传播要等
  `app.update()`,读它拿到的是上一帧的角度,拖动时卡片会慢半拍跟在粒子后面。
...
- **锚点不取封面平面的几何角**。相机竖向 45°、平面在镜头前 6.2,竖屏(小米13 是
  0.45)横向可视半宽只有约 1.16,而平面半边长 2.4:角早就出画了,锚在那儿的卡片
  一拖就一闪一灭。0.8 这个半径转到任何角度都还在竖屏画面内。
- **逐片元,不是逐物体**:横跨锚点平面的物体会被平面切开。CPU 侧按物体排序做不到,而
  「UI 整层贴在 canvas 上」的方案在原理上就做不到 —— 这正是这条路线买到的东西。
- **代价是几何提交两遍**。粒子用一个方片而非球体,正是为了让这一遍付得起。锚点转出
  画面的那些帧,UI 侧把 `needs_occluder` 置假、这台相机整个关掉(用的是上一帧的锚点 ——
  锚点是渲染的产物,拿不到同帧的答案,差一帧看不出来)。再要省就得改成采样深度纹理的
  一个全屏 pass,那需要自建渲染图节点与 WGSL,现在不值。
