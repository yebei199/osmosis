...
fn adding_a_device_keeps_the_group_playing_and_joins_it() {
    let mut session =
        grouped(vec![Output::Local], Output::Local);
    ...
    assert!(
        !session.holds_transport(),
        "主端一直在响，控制不压"
    );

    let started = session.on_ack(
        &device("pc1"),
    ...
    assert_eq!(
        started,
        vec![start("op", device("pc1"), 61_500)]
    );
...
fn removing_a_follower_stops_only_it() {
    ...
    let committed = session.on_ack(
        &device("pc1"),
        &ack("op", OperationPhase::Stopped, Some(70_000)),
        NOW + 100,
    );
    assert_eq!(committed, vec![commit("op", None)]);
...
fn replacing_the_group_starts_the_new_members_together_from_the_stop_anchor()
 {
    ...
    assert_eq!(
        ...
        vec![
            start("op", device("b"), 63_000),
            start("op", device("c"), 63_000)
        ]
    );
    assert!(
        session.holds_transport(),
...
fn a_slow_member_joins_later_instead_of_holding_the_group()
{
    ...
    assert_eq!(
        session.tick(NOW + 100 + READY_GRACE_MS - 1),
        Vec::new()
    );
    assert_eq!(
...
fn a_member_that_never_prepares_is_left_out_of_the_commit()
{
    ...
    assert!(
        session
            .failure()
            .is_some_and(|why| why.contains("设备 c")),
        "{:?}",
        session.failure()
    );
}
...
fn a_member_that_cannot_get_the_media_is_reported_and_left_out()
 {
    ...
    assert!(
        session
            .failure()
            .is_some_and(|why| why.contains("设备 pc1")
                && why.contains("取不到媒体")),
        "{:?}",
        session.failure()
    );
