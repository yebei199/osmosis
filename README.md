...
产出 `osmosis-desktop-x86_64-linux`、`io.github.osmosis.desktop`、
`io.github.osmosis.svg` 与 `sha256sums.txt` 到该 tag 的 GitHub Release;
NixOS 侧由 `nixos_config` 的 `fetchurl` 钉住这份产物消费。
...
形状由 `contract` crate 定义。歌单与红心归我们:`/playlists` 只给置顶的「我的喜欢」
与本地歌单,网易云只是音源
(见 [`docs/adr/0033`](docs/adr/0033-playlists-and-likes-are-ours.md))。gRPC 的价值在 axum↔bang-dream 那一段 ——
