...
fn android_main(app: slint::android::AndroidApp) {
    ...
    // 下面这段 3D 分派与 apps/desktop **逐字相同**,故意不抽:签名漂移编译器会一起报错。
    // web 端只差一处:它是紧凑版式、没有宽版式侧栏,故仍走 ui::run_with_renderer(不带导航
    // 选中器);desktop/android 走 run_with_renderers 多驱动一个 NavGlassPass。改一端记得同步。
