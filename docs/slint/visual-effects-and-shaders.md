...
### 落地过:3D 页的热调工具条(2026-07-29 已随该页一并删除)

这套架构在本仓库真跑过,代码是 `crates/render3d/src/glass.{rs,wgsl}`,寄生在 3D 演示页的
热调工具条上。3D 演示页删掉后它没有别的消费者,一起删了 —— 下面记的是当时的做法和结论,
要看代码去 git 历史。

- `glass.wgsl` —— 圆角矩形 SDF + 32 点 Vogel 螺旋圆盘模糊 + 沿 SDF 梯度的边缘折射 + 淡染。
  玻璃之外原样透传(实测与 bevy 直出逐字节相同)。
- `glass.rs` —— 最终形态是 bevy 的一个 `FullscreenMaterial`,跑在 bevy 自己的 ping-pong
  纹理上,结果由 bevy 写进那张共享纹理。
...
对照组就在同一个 app 里:Home 页的玻璃**没有**这一层(背后是 Slint 自己画的极光),
只能靠半透明透色,不会模糊。两者并排,就是这篇文档整个论点的实物证明。

两条踩出来的结论,比代码本身更值得留:

**一、后处理要长进 bevy 的渲染管线,别在旁边自己起一趟。** 最早的实现自建一张输出纹理、
自己 `queue.submit`,再把那张纹理交给 Slint 采样。改成 `FullscreenMaterial` 之后,链上
只剩一张纹理、一个写入方,净删 239 行,也不再每帧重建 view 与 bind group。凡是能表达成
bevy 材质的后处理,都别自己维护第二条提交路径。

**二、这次重构一度被当成安卓闪黑的修复,那个说法不成立。** 把玻璃整个关掉,闪黑照旧出现,
比率还略高(9/1256 对 6/1534)。性能重构与渲染时序 bug 长得像,是因为两者都"改了提交顺序",
但要判定因果只能靠关掉变量重测。排查全过程见 `docs/wasm/native-regression-2026-07-19.md`。
