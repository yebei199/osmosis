...
**局部重绘。**安卓这条路径上它根本没开。安卓带 `slint/unstable-wgpu-29`,
`AndroidWindowAdapter` 走 `SkiaRenderer::default_wgpu_29`,而 `wgpu_29_surface` 没有覆写
`Surface::use_partial_rendering()`,拿的是 trait 默认值 `false`
(`internal/renderers/skia/lib.rs:1102`),每帧全窗口重画。脏区与缓冲区轮换那条线索到此为止。

### 原因:Skia 的绘制与 wgpu 的 present 之间没有顺序

Skia 通过 `as_hal` 从 wgpu 里掏出裸 `VkQueue`,直接往交换链图像上画,wgpu 完全看不见这次
提交。它随后发出的 present 等的是一个 Skia 从未 signal 的 semaphore,于是扫描输出与绘制
之间没有任何顺序约束。合成器可以在 GPU 还没写完时就把这张图拿走,取到的就是上半截有新内容、
下半截还空着的一帧,切口每帧位置不同。

在 present 之前阻塞 CPU 直到 Skia 的 GPU 工作完成(`gr_context.flush_submit_and_sync_cpu()`,
见 slint fork 的 `probe/wgpu-present-sync`),半截帧消失:

| 条件 | 帧数 | 半截帧 | 比率 | 帧率 | 最暗帧亮度 |
| --- | --- | --- | --- | --- | --- |
| 基线(fork 的 dev) | 10895 | 9 | 0.083% | 92fps | 37.2 |
| 探针(present 前同步 CPU) | 11713 | 0 | 0% | 50fps | 64.3 |
| 回退探针,重装 dev | 10943 | 14 | 0.13% | 93fps | 37.2 |

两组 dev 的最暗帧都落在 37.2,与 07-19 抓到的半截帧同一档。探针那 11713 帧里最暗的一帧是
64.3,中位亮度 68.7,没有一帧接近半截帧的形态。按两组 dev 合计的 23/21838(0.105%),探针
的样本量里期望出现 12.3 次,实测 0 次,p 约 5e-6。

第三行是回退验证:把 patch 指回 dev、重编重装之后半截帧照旧出现,排掉了「探针期间设备或
系统状态变了」这类解释。

这个探针不是修复方案:它每帧要一次完整的 GPU 往返,帧率从 92 掉到 50。真正的修复要在 GPU
一侧建立顺序,让 wgpu 的 present 等在 Skia 那次提交 signal 的 semaphore 上。这在本仓库里
改不动,得改 slint fork 的 `wgpu_29_surface`。

有一个混淆项没排掉:探针同时把帧率降了一半,窗口变小本身也会压低撞车概率。要分开这两者,
得做出一个保持帧率的顺序修复再测一次。

### 测量时必须校验应用还活着

第一次探针测量报出 33.93% 的半截帧,是假的。应用在录制中途被杀,后面大半段录的是 MIUI 桌面,
亮度分布整个变样(中位 61.5,最暗 23.5,与真实的 68.7/37.2 两档都对不上)。

测量现在收进 `just android-flicker <标签> [秒数]`,它在录制前后各取一次 `pidof`,对不上就
作废。之前两次这个脚本都写在会话临时目录里,会话一关就没了,写第三遍时才收进仓库。

本节 07-20 深夜这批数字的基线比率 0.083%,低于同日早些时候的 0.39%,两批的拖动方式不同,
不要跨批直接比。同一批内的对照才有意义。
