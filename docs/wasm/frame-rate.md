...
**问题收敛成一句话:Slint 在 wasm 上每画一个界面元素,就要多花约 1ms 的 GPU 进程时间。**
十来个元素就是 14ms,超过 144Hz 的一拍(6.94ms),于是只能隔一拍出一帧。而这些时间既
不是光栅化(与像素量无关)也不是命令记录(`vkQueueSubmit` 只有 0.2ms/帧),它花在
Dawn 的 `Queue::Submit` 内部、所有子跨度之外。
