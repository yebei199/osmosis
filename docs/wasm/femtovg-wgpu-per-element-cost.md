...
**本项目的 3D 页:39fps → 108.7fps**(帧率达到同机 WebGPU 天花板的 75%,`frame-rate.spec`
...
现在每个 draw 调一次 `queue.write_buffer`,5000 个矩形就是 5000 条 wire 命令 —— 这解释了
为什么 1000 与 5000 个矩形都卡在 8ms(不再随元素数长,但仍有每 draw 的固定开销)。
把 uniform 攒进 CPU 侧一个 `Vec`、每帧只上传一次,应该还能再压一截。
