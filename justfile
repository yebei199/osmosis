...
font-subset:
    nix-shell -p python3Packages.fonttools --run "pyftsubset \
      ...
      --text='未查询中服务端协议失败网络错误响应格式版本不匹配本机·…,点一首歌开加载正在播放音频设备流解码始端暂不支持同播推给台收听连接上信令载荷线消息队列完了没有其他选个用缓冲置外观主题深浅色跟随系统账号退出登录已快捷桌面按处理生效当前页起月过续次天只读云待活来它就换批荐动态极光钮画零重绘拖翻找双击滚轮进度卡墙表视图输被遥控管停状期到下这还经存乐试片段整可写的条目句柄认联回那制' \
