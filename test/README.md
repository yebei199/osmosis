...
组内那种情况(#142:本机是主端,组里还有别的设备)再设 `FOLLOWER_PORT` 为跟随端的
MCP 端口,通过之后还要看到跟随端控制条上的曲名(`PlayerBar::title`)30 秒内换成主端这一首:

```sh
FOLLOWER_PORT=8090 test/pick-e2e.sh list   # 桌面是主端,手机跟着
```
...
三条场景各对一条验收:被控端信令断 `CUT` 秒(缺省 5)再放开,横幅一直在、重连回来
仍被遥控端遥控、服务端记了「组成员租约内回来了」(#142 推翻了 #118 原来那条「断线就撤锁、
恢复后遥控端回本机」);掐住遥控端的信令让它名册停住,再让目标下线、去接管它,恢复后接管
...
`LEASE` 缺省 60,要与服务端 `control::LEASE` 一致(#142 从 30 放宽)。ns 实例的「在出声」看播放器自己的
